...
class Person:
    ...
    def update(self,column,new_value):
        command = f"""UPDATE {table.table_name} SET {column}={new_value}
...
table.connect()
# table.create_table()
vova = Person(table,1,'zinko17','zinkpmail.ru','lalaland',50001,'developer')
# vova.insert_table()

aliya = Person(table,2,'aliya','djkfmls;.ru','lalaland',50002231,'developer')
# aliya.insert_table()
aliya.update('salary',-1)
aliya.select('between',-2,52000)